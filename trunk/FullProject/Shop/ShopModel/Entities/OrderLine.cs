...
namespace ShopModel.Entities
{
    public class OrderLine
    {
        ...
        public void Create(SqlTransaction transaction)
        {
            ...
            var command = new SqlCommand(String.Format("INSERT INTO {0} (idOrder,idRecept) VALUES (@idOrder,@idRecept)", TableName), ConnectionDb.Connection,transaction);
        ...
        public void Update(SqlTransaction transaction)
        {
            ...
            var command = new SqlCommand(String.Format("UPDATE {0} SET idOrder=@idOrder,idRecept=@idRecept WHERE id=@id", TableName), ConnectionDb.Connection,transaction);
        ...
        public static OrderLine Load(int id)
        {
            ...
            var adapter = new SqlDataAdapter(String.Format("SELECT id,idOrder,idRecept FROM {0} WHERE id={1}", TableName, id), ConnectionDb.Connection);
            var ds = new DataSet();
